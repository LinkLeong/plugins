...
public class Logger {
	 ...
	 * log tag
	 */
	private String tagName = "ShopLogger";// tag name
	...
	public void i(String format, Object... args) {
		if (logLevel <= Log.INFO && MyString.isShowLog) {
			lock.lock();
			try {
	...
	public void v(String format, Object... args) {
		if (logLevel <= Log.VERBOSE  && MyString.isShowLog) {
			lock.lock();
			try {
				String message = createMessage(getInputString(format, args));
	...
	public void d(String format, Object... args) {
		if (MyString.isShowLog && logLevel <= Log.DEBUG  ) {
			lock.lock();
			try {
				String message = createMessage(getInputString(format, args));
	...
	public void e(String format, Object... args) {
		if (logLevel <= Log.ERROR  && MyString.isShowLog) {
			lock.lock();
			try {
				String message = createMessage(getInputString(format, args));
	...
	public void w(String format, Object... args) {
		if (logLevel <= Log.WARN  && MyString.isShowLog) {
			lock.lock();
			try {
				String message = createMessage(getInputString(format, args));
